...
# 적 캐릭터
enemy = pygame.image.load("/python lecture(utilize)/making game/resource/ex resource/enemy.png")

# 적 캐릭터의 크기
enemy_size = enemy.get_rect().size # 이미지의 크기를 구해온다.
enemy_width = enemy_size[0] # 적 캐릭터의 가로크기
enemy_height = enemy_size[1] # 적 캐릭터의 세로크기

# 캐릭터의 위치
enemy_x_pos = (screen_width / 2) - (enemy_width / 2) # 화면 가로 절반 크기에 해당 하는 곳에 위치
enemy_y_pos = (screen_height / 2) - (enemy_height / 2) # 화면 세로  가장 아래에 위치
