package src.multiCampus.exception.runtime;
...
public class Exception {
    ...
    * Unchecked Exception : 코드로 예외발생을 차단할 수 있기 때문에, 예외처리를 강제하지 않는 Exception
    ...
    예외처리 방법
        (1) try-catch 문 : try 블록에 예외가 발생할 수 있는 코드를 작성, catch 블록에 예외처리에 필요한 코드를 작성
        (2) try-catch-finally : finally 블록에 예외 발생 여부와 상관없이 수행 되어야 하는 코드를 작성
    */
    ...
    // try 문 안에서 여러 종류의 예외가 발생할 때 처리하는 방법
    public void studyMultiException() {
        ...
        try {
            ...
            // NullPointerException : 레퍼런스에 주소값 대신 null 이 들어가 있는데 참조를 시도할 경우 발생하는 예외
            String nullStr = null;
            nullStr.equals("a");
...
//		}
            ...
            // 다형성을 이용해 try 블록에서 발생하는 모든 예외에 대해 일괄적인 예외처리를 적용
//		} catch (Exception e) {
//			System.out.println("예외가 발생했습니다!");
//			System.out.println(e.getMessage());
//		}
