...
# 파일 추가
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", filetypes=(("PNG 파일", "*.png"), ("모든 파일", "*.*")), initialdir="C:/") # 최초에 C:/ 경로를 보여준다.
...
def merge_image():
    ...
    # heights = [x.size[0] for x in images]
    ...
    dest_path = os.path.join(txt_dest_path.get(), "nado_photo.jpg") # 저장 될 파일 이름이다.
...
txt_dest_path.pack(side="left", fill="x", padx=5, pady=5, expand=True, ipady=4) # 높이 변경

btn_dest_path = Button(path_frame, text="찾아보기", padx=5, pady=5, width=10, command=brows_dest_path)
